		...
		while (mIsRunning && !mVideoOutputDone) {
			...
			if (decoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
				...
				if (mVideoBufferInfo.size > 0) {
					if (!frameCallback.onFrameAvailable(mVideoOutputBuffers[decoderStatus], mVideoBufferInfo.presentationTimeUs, true))
		...
		while (mIsRunning && !mAudioOutputDone) {
			...
			if (decoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
				...
				if (mAudioBufferInfo.size > 0) {
					if (!frameCallback.onFrameAvailable(mAudioOutputBuffers[decoderStatus], mAudioBufferInfo.presentationTimeUs, false))
